...
# from .utils_dataset import get_subject, get_dataset_variable_name
...
class Dataset:
    def __init__(self, cfg: DictConfig, verbose=True):
        ...
        if self.cfg.type == "zstruct":
            ...
            raise ValueError(f"Invalid dataset type: {self.cfg.type}")
    ...
    def _initialize_zstruct(self):
        ...
        self.nwb_compression = {
            "type": self.cfg.nwb.compression.type,
            "options": self.cfg.nwb.compression.options,
        }
    ...
    def set_server_directory(self, server_dir):
        ...
        """
        self.server_dir = get_server_data_path(
            is_monkey=self.is_monkey, custom_server_folder=server_dir
        )

    # TODO: add the functionality that checks if the NWB file exists and if yes it just loads it
    # def load_data(
    #     self, subject_name=None, date=None, data_path=None, runs=None, num_channels=96
    # ):
    def load_data(self):
        if self.cfg.type == "zstruct":
            ...
            if self.cfg.nwb.save:
                ...
                if self.cfg.data_path is None:
                    path = os.path.join(
                        ...
                        self.cfg.date,
                        f"Run-{run:03d}",
                        f"{self.cfg.subject}_{self.cfg.date}_Run-{run:03d}.nwb",
                    )
    ...
    def save_data(self, fields):
        # TODO
        if self.cfg.type == "zstruct":
            self._save_data_zstruct(fields)
        elif self.cfg.type == "nwb":
            self._save_data_nwb(fields)
        else:
            raise ValueError(f"Invalid dataset type: {self.cfg.type}")
    ...
    def extract_features(self, fields, params=[]):
        ...
        assert (
            len(fields) > 0
        ), "At least one field must be specified for the feature extraction"
        assert (
            "bin_size" in params
        ), "Parameter 'bin_size' must be specified for the feature extraction"

        bin_size = params["bin_size"]
        behav_lag = params["behav_lag"] if "behav_lag" in params else None
        overlap = params["overlap"] if "overlap" in params else 0
        remove_first_trial = (
            params["remove_first_trial"] if "remove_first_trial" in params else False
        )

        if not np.isscalar(bin_size) or not (
            type(bin_size) == int or type(bin_size) == float
        ):
            raise Exception("Parameter bin_size must be a numeric scalar!")

        if behav_lag and (
            not np.isscalar(behav_lag)
            or not (type(behav_lag) == int or type(behav_lag) == float)
        ):
        ...
        for run in self.runs:
            ...
            timestamps = (
                self.nwb_files[run]
                .processing["neural_data"]
                .get_data_interface("neural_features")
                .timestamps[:]
            ...
            if "sbp" in fields:
                ...
                sample_width = (
                    self.nwb_files[run].acquisition["SampleWidth"].data[:]
                )  # TODO: change this when the normalization of the names will be done

            if "fingers_kinematics" in fields:
                fingers_pos_data = (
                    self.nwb_files[run]
                    .processing["behavior"]
                    .get_data_interface("fingers_position")
                    .data[:]
                )
            ...
            if "trials_filter" in params:
                ...
                if "blank_trial" in filter_conditions:
                    trials = trials[
                        trials["BlankTrial"] == filter_conditions["blank_trial"]
                    ]
                if "trial_success" in filter_conditions:
                    trials = trials[
                        trials["TrialSuccess"] == filter_conditions["trial_success"]
                    ]
            ...
            trial_numbers = trials[
                "TrialNumber"
            ].values  # TODO: standardize the name of TrialNumber to trial_number

            # finding the non-consecutive runs of trials - used later for consecutive features extraction
            trials_runs = np.where(np.diff(trial_numbers) != 1)[
                0
            ]  # non-consecutive run are delimited where the trial-num difference is not 1
            trials_runs = np.concatenate(
                [[-1], trials_runs, [num_trials - 1]]
            )  # adding first and last trial index
            ...
            for run_id in range(len(trials_runs) - 1):
                ...
                num_bins = int(
                    np.fix((desidered_last_bin_time - start_time) / bin_size)
                )
                ...
                if behav_lag:
                    ...
                    bins_start_digit_lag = bins_start_digit_lag[
                        bins_start_digit_lag >= 0
                    ]
                ...
                for field in fields:
                    if (
                        field == "sbp" or field == "mav"
                    ):  # Spiking Band Power or Mean Absolute Value - TODO: maybe change the name of these two fields
                        sbp_run_data = neural_data[start_time_index:stop_time_index, :]
                        sample_width_run = sample_width[
                            start_time_index:stop_time_index
                        ]

                        bins_start = (
                            bins_start_digit_lag if behav_lag else bins_start_digit
                        )
                        bins_stop = (
                            bins_stop_digit_lag if behav_lag else bins_stop_digit
                        )

                        run_features = calc_bins_sbp(
                            sbp_run_data, sample_width_run, bins_start, bins_stop
                        )

                    elif (
                        field == "fingers_kinematics"
                    ):  # Fingers kinematics (position, velocity, acceleration)
                        fingers_pos_run_data = fingers_pos_data[
                            start_time_index:stop_time_index, :
                        ]
                        ...
                        run_features = calc_bins_fingers_kinematics(
                            fingers_pos_run_data, bins_start, bins_stop, num_fingers
                        )
                        ...
                        raise ValueError(
                            f"Field '{field}' not supported for feature extraction yet"
                        )

                    if features[field] is None:
                        features[field] = run_features
                    else:
                        features[field] = np.concatenate(
                            (features[field], run_features), axis=0
                        )
    ...
    #             group=self.electrode_group, location=self.electrode_group.location
    #         )
    ...
    #     data_dict = read_xpc_data(
    #         contents, run_path, num_channels=self.num_channels, verbose=self.verbose
    #     )

    #     # initializing the nwb modules and columns
    #     self.__initialize_nwb_modules(run)
    #     time_series_dict = self.__initialize_nwb_columns(
    #         run, data_dict
    ...
    #     num_times = len(
    #         data_dict[-1]["ExperimentTime"]
    #     )  # num of times for last trial - used to understand if a variable is a time-series
    #     num_total_times = sum(
    #         len(data_dict[trl_idx]["ExperimentTime"]) for trl_idx in range(num_trials)
    #     )
    ...
    #         elif (
    #             not is_collection(data_dict[0][key])
    #             or len(data_dict[-1][key]) != num_times
    #         ):
    ...
    #         (total_times, self.num_channels),
    #         dtype=data_dict[0][self.neural_var_name].dtype,
    #     )
    #     behavior_data = np.empty(
    #         (total_times, num_behavior_vars),
    #         dtype=data_dict[0][self.behavior_var_name].dtype,
    #     )
