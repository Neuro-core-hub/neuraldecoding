...
from neuraldecoding.model.neural_network_models.Adaline import Adaline

class AdalineKF(KalmanFilter):
    def __init__(self, model_params):
        """
        Constructs a Kalman Filter decoder object that uses Adaline to calculate C. 
        
        Parameters:
            model_params (dict) containing keys: 'append_ones' (bool) which specifies whether or not to add column of ones for bias, 
        ...
        self.A, self.W, self.Q = None, None, None
        self.Pt = None
        self.append_ones_y = model_params.get("append_ones", True)
        ...
        self.num_channels = model_params.get("num_channels")
        self.num_outputs = model_params.get("num_outputs")
        self.history = model_params.get("history")
        self.device = model_params.get("device", "cpu")

        self.adaline_filepath = model_params.get("filepath")
        
        adaline_model = Adaline({ "input_size": self.num_outputs, "num_outputs": self.num_channels, "history": self.history, "device": self.device, "append_ones": self.append_ones_y })
        adaline_model.load_model(self.adaline_filepath)
        if adaline_model.history != 1:
            raise Exception("History length must be 1 for trained Adaline model")

        y_scaler = adaline_model.neural_scaler
        x_scaler = adaline_model.behavior_scaler
        mu_x = x_scaler.mean_.reshape(-1, 1)
        sigma_x = x_scaler.scale_.reshape(-1, 1)
        mu_y = y_scaler.mean_.reshape(-1, 1)
        sigma_y = y_scaler.scale_.reshape(-1, 1)
       
        C_norm = adaline_model.linear.weight.data.cpu().numpy()
        W_norm = C_norm[:, :-1] 
        b_norm = C_norm[:, -1].reshape(-1, 1)

        W_denorm =  W_norm * (sigma_y @ (1/sigma_x).T)
        b_denorm = (sigma_y * b_norm) - (W_denorm @ mu_x)
        self.C = np.hstack((W_denorm, b_denorm))
    ...
    def train_step(self, input_data: tuple[np.ndarray, np.ndarray]):
        """
        Trains the matrices in the model. If append_ones is true, a column of ones is added to calculate bias. 
