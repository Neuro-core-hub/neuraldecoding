...
class NNTrainer(Trainer):
    ...
    #     while True:
    #         if (self.num_epochs is not None and epoch > self.num_epochs) or (self.max_iters is not None and iteration >= self.max_iters):
    #             break
    #         # Train
    ...
    #             self.optimizer.zero_grad()
    #             # Train step
    #             loss, y, yhat = self.model.train_step(x.to(self.device), y.to(self.device), self.optimizer, self.loss_func, clear_cache = self.clear_cache)

    #             train_loss = loss.item()
    #             # TODO: DO this on validation loss, not train loss
    #             if self.take_best and train_loss < best_val_loss:
    #                 best_val_loss = train_loss
    #                 best_state_dict = copy.deepcopy(self.model.state_dict())
    #                 epoch_best = epoch
    #                 iteration_best = iteration
                
    #             train_predictions = yhat.detach().cpu().numpy()
    #             train_targets = y.detach().cpu().numpy()
    #             if(self.clear_cache):
    #                 del y, yhat
                    
    #             if iteration % self.print_every == 0:
    #                 print(f"Epoch {epoch}, Iteration {iteration}, Train Loss: {loss.item():.4f}")
    #                 # Validate
    #                 val_loss, val_all_predictions, val_all_targets = self.validate_model()
    ...
    #                         metric_param = self.metric_params.get(metric, None)
    #                         metric_class = getattr(neuraldecoding.utils.eval_metrics, metric)
    #                         self.logger[metric]['train'].append(metric_class(train_predictions, train_targets, metric_param))
    #                         self.logger[metric]['valid'].append(metric_class(val_all_predictions, val_all_targets, metric_param))
    #                 # Logging
    #                 # self.save_print_log(epoch, iteration, train_loss, val_loss)
    #             iteration += 1
    #         epoch += 1
    #     if self.take_best and best_state_dict is not None:
    #         self.reload_model(best_state_dict, epoch_best, iteration_best, best_val_loss)
    #     return self.model, self.logger
