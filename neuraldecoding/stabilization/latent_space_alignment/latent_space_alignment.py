...
class CCAAlignment(LatentSpaceAlignment):
    # This is different than the base latent space alignment class because it aligns latent data, not loading matrices
    def __init__(self, cfg): 
        """ Initialize latent space alignment
        ...
        self.ndims = cfg.ndims
        self.dim_red_method = getattr(neuraldecoding.stabilization.latent_space_alignment.dim_red, cfg.dim_red_method.type)(**cfg.dim_red_method.params)
        self.alignment_method = getattr(neuraldecoding.stabilization.latent_space_alignment.alignment, 'CCA')()
        self.fpath = cfg.fpath

        
    def fit(self, data):
        """Train the CCAAlignment. Sets day_0 latent space and returns latent space
        ...
        lm, args = self.dim_red_method.calc_lm(data)
        latent_ds = self.dim_red_method.reduce(data, lm, args)
        self.alignment_method.set_baseline(latent_ds)
    ...
    def extract_latent_space(self, data):
        ...
        Returns:
            ls (numpy array of shape [ndims x timepoints]): alignesd latent space
        """    
        
        lm, args = self.dim_red_method.calc_lm(data)
        ls = self.dim_red_method.reduce(data, lm)
        aligned_ls = self.alignment_method.align(ls)
